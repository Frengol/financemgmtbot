...
class TestAdminApiHelperCoverage:
    ...
    @pytest.mark.asyncio
    async def test_listar_gastos_admin_uses_database_flow_when_auth_test_database_mode_is_enabled(self):
        actor = {"id": "u1", "email": "admin@example.com"}
        query = MagicMock()
        query.execute.return_value = MagicMock(data=[{"id": "tx-qa-1"}])
        gastos = MagicMock()
        gastos.select.return_value.order.return_value = query
        ...
        with patch.object(admin_transactions, "autenticar_admin_request", return_value=(actor, None)), \
            ...
            async with main.app.test_request_context("/api/admin/gastos"):
                response = admin_api.listar_gastos_admin()

        assert response.status_code == 200
        assert query.execute.called
