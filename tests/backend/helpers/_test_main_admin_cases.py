...
class TestAdminRoutes:
    ...
    @pytest.mark.asyncio
    async def test_admin_auth_rate_limit_happens_before_supabase_token_validation(self):
        auth_response = MagicMock()
        auth_response.user = {"id": "user-1", "email": "admin@example.com"}
        get_user = MagicMock(return_value=auth_response)

        async with main.app.test_client() as client:
            with patch.object(admin_auth, "ADMIN_AUTH_RATE_LIMIT", 1, create=True), \
                 patch.object(admin_auth, "ADMIN_AUTH_RATE_WINDOW_SECONDS", 60, create=True), \
                 patch.object(admin_auth.supabase.auth, "get_user", get_user), \
                 patch.object(admin_auth, "_lookup_admin_user", return_value={"user_id": "user-1", "email": "admin@example.com"}), \
                 patch.object(admin_auth, "ADMIN_EMAILS", frozenset({"admin@example.com"})), \
                 patch.object(admin_auth, "ADMIN_USER_IDS", frozenset({"user-1"})), \
                 patch.object(admin_auth, "auth_test_mode_enabled", return_value=False):
                first = await client.get("/api/admin/me", headers={"Authorization": "Bearer header.payload.signature"})
                second = await client.get("/api/admin/me", headers={"Authorization": "Bearer header.payload.signature"})

        assert first.status_code == 200
        assert second.status_code == 429
        payload = await second.get_json()
        assert payload["code"] == "ADMIN_AUTH_RATE_LIMITED"
        get_user.assert_called_once_with("header.payload.signature")
    ...
    @pytest.mark.asyncio
    async def test_runtime_meta_rate_limit_keeps_metadata_endpoint_cheap(self):
        async with main.app.test_client() as client:
            with patch.object(observability_routes, "META_RUNTIME_RATE_LIMIT", 1, create=True), \
                 patch.object(observability_routes, "META_RUNTIME_RATE_WINDOW_SECONDS", 60, create=True):
                first = await client.get("/api/meta/runtime")
                second = await client.get("/api/meta/runtime")

        assert first.status_code == 200
        assert second.status_code == 429
        payload = await second.get_json()
        assert payload["code"] == "META_RUNTIME_RATE_LIMITED"
