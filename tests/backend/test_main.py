...
from helpers._test_main_admin_cases import TestAdminRoutes, TestAdminValidationCoverage, TestObservabilityCoverage
from helpers._test_main_security_cases import TestSecurityCoverage
...
class TestFormatarResumoRegistrosSalvos:
    def test_contains_common_header_and_saved_records(self):
        registros = [
            {
                "data": "2026-05-09",
                "valor": 25.0,
            ...
            },
            {
                ...
                "descricao": "Cerveja (Cupom)",
                "metodo_pagamento": "Pix",
                "conta": "Nubank",
            },
        ]

        msg = core_logic.formatar_resumo_registros_salvos(registros)

        assert "Cupom salvo" in msg
        assert "Data: 2026-05-09" in msg
        assert "Pagamento: Pix" in msg
        assert "Banco/conta: Nubank" in msg
        assert "Registros salvos" in msg
        assert "R$ 25.00 | Essencial > Mercado" in msg
        assert "R$ 15.50 | Lazer > Bebidas Alcoólicas" in msg
        assert "Total" in msg
        assert "R$ 40.50" in msg
        assert "Registros: 2" in msg
        assert "Arroz (Cupom)" not in msg
        assert "Cerveja (Cupom)" not in msg
...
class TestGerarTextoEdicao:
    ...
    def test_orders_items_by_category_then_name(self):
        dados_lote = {
            "metodo_pagamento": "Pix",
            "conta": "Nubank",
            "desconto_global": 0.0,
            "itens": [
                {"nome": "Sabonete", "valor_bruto": 8.0, "desconto_item": 0.0, "categoria": "Cuidados Pessoais"},
                {"nome": "Cerveja", "valor_bruto": 15.0, "desconto_item": 0.0, "categoria": "Bebidas Alcoólicas"},
                {"nome": "Arroz", "valor_bruto": 25.0, "desconto_item": 0.0, "categoria": "Mercado"},
                {"nome": "Banana", "valor_bruto": 6.0, "desconto_item": 0.0, "categoria": "Mercado"},
            ],
        }

        text = core_logic.gerar_texto_edicao(dados_lote)

        assert text.index("[Bebidas Alcoólicas] Cerveja") < text.index("[Cuidados Pessoais] Sabonete")
        assert text.index("[Cuidados Pessoais] Sabonete") < text.index("[Mercado] Arroz")
        assert text.index("[Mercado] Arroz") < text.index("[Mercado] Banana")
...
class TestRemoverTecladoMensagemTelegram:
    @pytest.mark.asyncio
    async def test_removes_inline_keyboard_without_editing_text(self, mock_http_client):
        await telegram_service.remover_teclado_mensagem_telegram(123, 456)

        payload = mock_http_client.post.call_args[1]["json"]
        assert payload["chat_id"] == 123
        assert payload["message_id"] == 456
        assert payload["reply_markup"] == {"inline_keyboard": []}
        assert "editMessageReplyMarkup" in mock_http_client.post.call_args[0][0]
...
class TestProcessarTextoComLLM:
    ...
    async def test_returns_parsed_json(self):
        ...
        config.deepseek_client.chat.completions.create.assert_called_once()
        call_kwargs = config.deepseek_client.chat.completions.create.call_args.kwargs
        assert call_kwargs["model"] == "deepseek-v4-pro"
        assert call_kwargs["response_format"] == {"type": "json_object"}
        assert call_kwargs["extra_body"] == {"thinking": {"type": "disabled"}}
        assert "Retorne EXCLUSIVAMENTE este JSON" in call_kwargs["messages"][0]["content"]
...
class TestProcessarUpdateAssincrono:
    ...
    @pytest.mark.asyncio
    async def test_photo_rate_limit_blocks_download_and_ocr(self, mock_http_client):
        self._setup_supabase_idempotency()
        update = {
            "update_id": 1700,
            "message": {
        ...
        with patch.object(handlers, "CHAT_MEDIA_RATE_LIMIT", 0, create=True), \
             patch.object(handlers, "CHAT_MEDIA_RATE_WINDOW_SECONDS", 60, create=True), \
             patch("handlers.baixar_arquivo_telegram", AsyncMock(return_value=b"img")) as baixar, \
             patch("handlers.extrair_tabela_recibo_gemini", AsyncMock(return_value="Arroz 25")) as ocr:
            await handlers.processar_update_assincrono(update)
    ...
    async def test_intent_salvar_edicao_cupom(self, mock_http_client):
        ...
        assert any(
            ...
            and "R$ 25.00 | Essencial > Mercado" in m
            and "Arroz (Cupom)" not in m
            for m in msgs
        )
    ...
    @pytest.mark.asyncio
    async def test_text_ai_rate_limit_blocks_llm_call(self, mock_http_client):
        self._setup_supabase_idempotency()
        update = {
            "update_id": 1800,
            "message": {"chat": {"id": 123}, "from": {"id": 456}, "text": "cerveja 15"},
        }

        with patch.object(handlers, "CHAT_AI_RATE_LIMIT", 0, create=True), \
             patch.object(handlers, "CHAT_AI_RATE_WINDOW_SECONDS", 60, create=True), \
             patch("handlers.processar_texto_com_llm", AsyncMock(return_value={"intencao": "consultar"})) as llm:
            await handlers.processar_update_assincrono(update)
