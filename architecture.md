...
11. O frontend chama o backend em `/api/admin/*` apenas com `Authorization: Bearer <access_token>`; a correlação operacional permanece baseada em `requestId` e logs sanitizados, sem depender de headers públicos extras.
12. O backend faz o parsing do header `Authorization` com um parser dedicado de Bearer, preservando o JWT inteiro e rejeitando apenas formato inválido ou tamanho anormal; o sanitizador genérico de texto não deve ser aplicado ao token bruto para não truncar credenciais válidas.
