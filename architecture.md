...
**Versão:** V3.3.2 — *GitHub Pages com Sessão Supabase no Browser, Callback Canônico no Frontend e Relay Seguro de Compatibilidade*
...
17. O callback legado do backend (`GET /auth/callback`) passa a atuar apenas como relay de compatibilidade: ele preserva `hash` ou `query string` vindos do Supabase e redireciona o navegador para o callback do frontend, sem mais criar sessão cookie para o fluxo oficial do Pages.
