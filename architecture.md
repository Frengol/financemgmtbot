...
4. O GitHub Pages entrega o shell da SPA nessa rota via fallback `404.html`, e o `supabase-js` conclui a sessão oficial no navegador com `persistSession: true`, `detectSessionInUrl: true` e `flowType: 'pkce'`.
5. No GitHub Pages publicado, o contexto `useAuth` reflete exclusivamente a sessão oficial do navegador via `supabase.auth.getSession()` e `onAuthStateChange(...)`; o handshake `/api/admin/me` acontece apenas nesse contexto compartilhado, e o runtime publicado não depende mais de `GET /auth/session`.
...
5. **Camada Administrativa Segura:** `main.py` atua como composition root, `web_app/*` registra as superfícies HTTP e `admin_runtime/*` concentra autenticação bearer-only, CRUD de transações, fila de aprovações e escrita de trilha de auditoria.
6. **Persistência Privilegiada:** escrita sensível e leitura administrativa continuam passando pelo backend com `service_role` no Cloud Run, fora do navegador.
...
### 4.2 Supabase & RLS
* O acesso web é controlado por Supabase Auth como identidade upstream e como sessão oficial do navegador no GitHub Pages; o painel administrativo produtivo é bearer-only e não depende mais de sessão web cookie-based.
...
* O painel web produtivo não usa mais sessão web cookie-based nem cabeçalhos CSRF; a autenticação administrativa oficial é exclusivamente `Authorization: Bearer <access_token>` com validação server-side.
...
* O callback do frontend permanece focado em concluir a sessão oficial do Supabase no browser; diagnósticos de transporte continuam vindo de `browser_client_telemetry`, mas a autorização administrativa (`/api/admin/me`) fica exclusivamente fora do callback, no contexto global de auth.
...
* Em desenvolvimento:
  - `BASE_URL=/`
  - proxy `/api` e `/__test__` → backend local `127.0.0.1:8080`
...
* O Dashboard compartilha um único estado de periodo entre os widgets analiticos e expoe, nos blocos principais da tela, um popover com abas `Mes` e `Filtro`: a aba `Mes` mantém a grade mensal com acao `Total` (todos os gastos desde o primeiro registro) e a aba `Filtro` permite escolher mes inicial e final para recalcular KPIs e graficos em toda a tela; a hierarquia visual prioriza 4 cards superiores (`Saldo atual`, `Receitas do mes`, `Gastos do mes` e `Saldo do mes`), com `Saldo atual` sempre acumulado/all-time e os tres cards mensais presos ao mes de referencia, seguidos por um grafico principal de `Receitas`, `Gastos` e `Saldo liquido` no tempo e por blocos secundarios de `Gastos por categoria`, `Receitas por categoria` (ou fallback elegante sem entradas no periodo) e `Essencial vs lazer` com insights automaticos como maior gasto do mes, categoria dominante e leitura do estado financeiro; na fase final de polimento, o header mobile passou a usar CTA compacto para `Nova Transacao`, o chip `Online/Offline` ficou restrito ao desktop, os blocos secundarios permanecem empilhados em telas menores e o picker recebeu estados de foco/hover/seleção consistentes com o restante do sistema visual.
