...
O deploy padrao nao usa On-Demand Scanning para evitar custo por deploy. A seguranca de dependencias fica nos gates gratuitos de CI/local (`npm audit`, `pip-audit` com hashes e `gitleaks`), enquanto o Cloud Build continua usando `docker build --pull` e deploy por digest.
...
O `cloudbuild.yaml` builda a imagem com `--pull`, publica no Artifact Registry, resolve o digest publicado, resolve a versao numerica habilitada de cada secret antes do deploy, aplica essas versoes no Cloud Run, valida `/api/meta/runtime` e depois remove versoes antigas. Ele nao le valores de secrets e nao imprime payloads.
