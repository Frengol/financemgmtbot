...
from admin_runtime.recurring_expenses import (
    atualizar_despesa_recorrente,
    criar_despesa_recorrente,
    deletar_despesa_recorrente,
...
def register_admin_routes(app):
    ...
    @app.route("/api/admin/despesas-recorrentes", methods=["GET", "POST", "OPTIONS"])
    async def admin_recurring_expenses():
        if request.method == "OPTIONS":
            return "", 204
        if request.method == "GET":
            return listar_despesas_recorrentes()
        payload = await request.get_json(silent=True)
        return criar_despesa_recorrente(payload)

    @app.route("/api/admin/despesas-recorrentes/<expense_id>", methods=["DELETE", "PATCH", "OPTIONS"])
    async def admin_manage_recurring_expense(expense_id):
        if request.method == "OPTIONS":
            return "", 204
        if request.method == "PATCH":
            payload = await request.get_json(silent=True)
            return atualizar_despesa_recorrente(expense_id, payload)
        return deletar_despesa_recorrente(expense_id)
