...
META_RUNTIME_RATE_LIMIT = 120
META_RUNTIME_RATE_WINDOW_SECONDS = 60
...
def _normalize_client_telemetry_payload(payload: dict[str, Any]):
    ...
    return {
        ...
        "clientRequestId": _validate_safe_token(payload.get("clientRequestId"), field_name="client_request_id"),
...
def register_observability_routes(app):
    ...
    async def runtime_meta():
        ...
        rate_limited_response = rate_limited(
            "meta-runtime",
            (request.remote_addr or "").strip() or "unknown",
            limit=META_RUNTIME_RATE_LIMIT,
            window_seconds=META_RUNTIME_RATE_WINDOW_SECONDS,
            code="META_RUNTIME_RATE_LIMITED",
            message="Too many runtime metadata requests. Try again later.",
        )
            ...
            with_request_id(
                {
                    ...
                    "client_request_id": normalized_payload["clientRequestId"],
