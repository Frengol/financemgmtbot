...
function mapUpstreamAuthError(upstreamError: string, upstreamCode: string | null) {
  ...
  if (
    ...
    return 'Link de acesso inválido ou expirado. Solicite um novo magic link.';
  }

  return upstreamError || 'Não foi possível concluir o login com este link. Solicite um novo magic link.';
}
    ...
    const failLogin = async () => {
      ...
      setError(
        ensureSupportCodeInMessage(
          'Não foi possível concluir o login com este link. Solicite um novo magic link.',
          clientEventId,
        ),
      );
    };
