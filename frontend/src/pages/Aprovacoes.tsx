...
export default function Aprovacoes() {
  ...
  const fetchCache = async () => {
    ...
    try {
      ...
      setError(normalizeAdminPageError(fetchError, "Não foi possível carregar as aprovações agora."));
    }
  };
  ...
  const handleAprovar = async (item: PendingApprovalItem) => {
    ...
    try {
      ...
      setError(normalizeAdminPageError(approveError, "Não foi possível aprovar o cupom pendente."));
    } finally {
      setPendingId(null);
    }
  };
  ...
  const handleRejeitar = async (id: string) => {
    ...
    try {
      ...
      setError(normalizeAdminPageError(rejectError, "Não foi possível rejeitar o cupom pendente."));
    } finally {
      setPendingId(null);
    }
  };
            ...
            <div key={item.id} className="bg-white border text-sm rounded-xl p-5 shadow-sm hover:shadow-md transition">
              <div className="flex justify-between items-start mb-4">
                <span className="inline-flex items-center gap-1.5 px-2.5 py-0.5 rounded-full text-xs font-semibold bg-blue-50 text-blue-700">
                  ...
                  {preview.summary || (isReceiptBatch ? 'Cupom pendente' : 'Pendência administrativa')}
                </span>
                ...
                <div className="bg-slate-50 p-3 rounded-lg border border-slate-100 space-y-2">
                  ...
                  <ul className="text-xs text-slate-600 space-y-1">
                    ...
                    {!previewItems.length && !isReceiptBatch && (
                      <li className="text-slate-400 italic">Aguardando confirmação da exclusão segura.</li>
                    )}
                  </ul>
                </div>
              </div>
