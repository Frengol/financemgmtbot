...
import { formatAccountLabel, formatCategoryLabel, normalizeNatureLabel, type TransactionRecord } from "@/lib/transactions";
...
export default function Historico() {
  ...
  const fetchGastos = async () => {
    ...
    try {
      ...
      setError(normalizeAdminPageError(fetchError, "Não foi possível carregar o histórico agora."));
    }
    setFetching(false);
  };
  ...
  const handleDelete = async (id: string) => {
    if (confirm("Deseja mesmo excluir este registro? A ação não pode ser desfeita.")) {
      ...
      try {
        ...
        setError(normalizeAdminPageError(deleteError, "Não foi possível excluir o registro."));
      } finally {
  ...
  const columns = useMemo(() => [
    ...
    columnHelper.accessor('categoria', {
      ...
      cell: info => (
        <span className="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-slate-100 text-slate-800 border">
          {formatCategoryLabel(info.getValue() as string) || "N/A"}
        </span>
      ),
    }),
    ...
    columnHelper.accessor('conta', {
      header: 'Conta',
      cell: info => <span>{formatAccountLabel(info.getValue() as string) || 'Não informada'}</span>,
    }),
