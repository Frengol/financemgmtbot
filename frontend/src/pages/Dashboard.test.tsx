...
describe('Dashboard', () => {
  ...
  it('falls back to zeroed metrics when the request fails', async () => {
    mockGetTransactions.mockRejectedValue(new Error('Não foi possível carregar os dados agora. Código de suporte: req_dash_1'));

    render(<Dashboard />);

    expect(await screen.findByText(/Não foi possível carregar os dados agora/i)).toBeInTheDocument();
    expect(screen.getByText(/Código de suporte: req_dash_1/i)).toBeInTheDocument();
  ...
  it('uses the generic dashboard error when the rejection is not an Error instance', async () => {
    ...
    expect(await screen.findByText('Não foi possível carregar os dados agora.')).toBeInTheDocument();
  });
  ...
  it('retries the request after an error', async () => {
    const user = userEvent.setup({ advanceTimers: vi.advanceTimersByTime });
    mockGetTransactions
      .mockRejectedValueOnce(new Error('Não foi possível carregar os dados agora. Código de suporte: req_dash_retry'))
      .mockResolvedValueOnce({
    ...
    expect(await screen.findByText(/Código de suporte: req_dash_retry/i)).toBeInTheDocument();
  ...
  it('shows a re-login action instead of retry for malformed auth sessions', async () => {
    ...
    mockGetTransactions.mockRejectedValue(new ApiError(
      'Sua sessão de acesso é inválida. Faça login novamente. Código de suporte: req_dash_auth Detalhe: bearer_malformed',
      {
