...
describe('Historico', () => {
  ...
  it('shows the mapped backend error when the history cannot be loaded', async () => {
    mockGetTransactions.mockRejectedValue(new Error('Não foi possível carregar os dados agora. Código de suporte: req_hist_1'));

    render(<Historico />);

    expect(await screen.findByText(/Não foi possível carregar os dados agora/i)).toBeInTheDocument();
    expect(screen.getByText(/Código de suporte: req_hist_1/i)).toBeInTheDocument();
  });
  ...
  it('offers re-login instead of generic retry when the auth token is malformed', async () => {
    mockGetTransactions.mockRejectedValue(new ApiError(
      'Sua sessão de acesso é inválida. Faça login novamente. Código de suporte: req_hist_auth Detalhe: bearer_malformed',
      {
  ...
  it('offers re-login when deleting fails with malformed auth', async () => {
    ...
    mockDeleteTransaction.mockRejectedValue(new ApiError(
      'Sua sessão de acesso é inválida. Faça login novamente. Código de suporte: req_hist_delete_auth Detalhe: bearer_malformed',
      {
        code: 'AUTH_SESSION_TOKEN_MALFORMED',
        detail: 'bearer_malformed',
        status: 401,
  ...
  it('ignores refresh events while the session is unavailable and no rows are loaded', async () => {
    ...
    expect(mockGetTransactions).not.toHaveBeenCalled();
    expect(screen.queryByText('Não foi possível validar sua sessão. Entre novamente.')).not.toBeInTheDocument();
  });
  ...
  it('shows an auth loading error before deleting', async () => {
    ...
    await userEvent.click(screen.getAllByTitle('Excluir')[0]);
    expect(await screen.findByText('Sua autenticação ainda está sendo carregada. Tente novamente em alguns segundos.')).toBeInTheDocument();
  });
