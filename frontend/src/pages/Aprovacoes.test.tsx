...
describe('Aprovacoes', () => {
  ...
  it('renders delete confirmations and rejects them successfully with bearer-only auth state', async () => {
    ...
    expect(screen.getByText(/Aguardando confirmação da exclusão segura/i)).toBeInTheDocument();
  ...
  it('shows the mapped backend error when pending receipts cannot be loaded', async () => {
    mockGetPendingReceipts.mockRejectedValue(new Error('Não foi possível carregar os dados agora. Código de suporte: req_pending_1'));

    render(<Aprovacoes />);

    expect(await screen.findByText(/Não foi possível carregar os dados agora/i)).toBeInTheDocument();
    expect(screen.getByText(/Código de suporte: req_pending_1/i)).toBeInTheDocument();
  });
  ...
  it('offers re-login when pending approvals fail with malformed auth', async () => {
    mockGetPendingReceipts.mockRejectedValue(new ApiError(
      'Sua sessão de acesso é inválida. Faça login novamente. Código de suporte: req_pending_auth Detalhe: bearer_malformed',
      {
  ...
  it('offers re-login when rejection fails with malformed auth', async () => {
    ...
    mockRejectPendingReceipt.mockRejectedValue(new ApiError(
      'Sua sessão de acesso é inválida. Faça login novamente. Código de suporte: req_pending_reject_auth Detalhe: bearer_malformed',
      {
        code: 'AUTH_SESSION_TOKEN_MALFORMED',
        detail: 'bearer_malformed',
        status: 401,
  ...
  it('offers re-login when approval fails with malformed auth', async () => {
    ...
    mockApprovePendingReceipt.mockRejectedValue(new ApiError(
      'Sua sessão de acesso é inválida. Faça login novamente. Código de suporte: req_pending_approve_auth Detalhe: bearer_malformed',
      {
        code: 'AUTH_SESSION_TOKEN_MALFORMED',
        detail: 'bearer_malformed',
        status: 401,
  ...
  it('falls back to the generic loading error when the rejection is not an Error instance', async () => {
    ...
    expect(await screen.findByText('Não foi possível carregar as aprovações agora.')).toBeInTheDocument();
  });
  ...
  it('surfaces action errors when approval fails', async () => {
    ...
    mockApprovePendingReceipt.mockRejectedValue(new Error('Não foi possível aprovar agora. Código de suporte: req_approve_1'));
    ...
    expect(await screen.findByText(/Não foi possível aprovar agora/i)).toBeInTheDocument();
    expect(screen.getByText(/Código de suporte: req_approve_1/i)).toBeInTheDocument();
  });
});
