...
const ERROR_MESSAGES: Record<string, string> = {
  ADMIN_ACTION_FAILED: 'Não foi possível concluir a operação agora.',
  ADMIN_DATA_LOAD_FAILED: 'Não foi possível carregar os dados agora.',
  AUTH_ACCESS_DENIED: 'Seu usuário não está autorizado a acessar o painel.',
  AUTH_CONFIGURATION_INVALID: 'A configuração de login está indisponível no momento. Tente novamente mais tarde.',
  AUTH_SESSION_INVALID: 'Sua sessão expirou. Faça login novamente.',
  AUTH_SESSION_TOKEN_MALFORMED: 'Sua sessão de acesso é inválida. Faça login novamente.',
  AUTH_SESSION_STORAGE_UNAVAILABLE: 'O login está temporariamente indisponível. Tente novamente em instantes.',
  NETWORK_ERROR: 'Não foi possível conectar ao servidor agora. Verifique sua conexão e tente novamente.',
};
...
function buildSupportMessage({
  ...
  if (requestId) {
    supportMessageParts.push(`Código de suporte: ${requestId}`);
  } else if (clientEventId) {
    supportMessageParts.push(`Código de suporte: ${clientEventId}`);
  }
  if (detail) {
    supportMessageParts.push(`Detalhe: ${detail}`);
  }
  if (clientRequestId) {
    supportMessageParts.push(`Correlação: ${clientRequestId}`);
  }
  if (diagnostic) {
    supportMessageParts.push(`Diagnóstico: ${diagnostic}`);
  }
...
export async function apiRequest<T>(
  ...
  const allowLocalAdminBypassRequest = localDevBypassEnabled && isAdminRoute;

  if (accessToken) {
    headers.set('Authorization', `Bearer ${accessToken}`);
  } else if (isAdminRoute && !allowLocalAdminBypassRequest) {
