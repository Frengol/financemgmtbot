...
describe('adminApi', () => {
  ...
  it('surfaces backend error messages for failed requests', async () => {
    ...
    await expect(deleteTransaction('tx-1')).rejects.toThrow('Seu usuário não está autorizado a acessar o painel.');
  });
  ...
  it('preserves short safe auth details for malformed bearer sessions', async () => {
    ...
    await expect(getTransactions()).rejects.toThrow(
      'Sua sessão de acesso é inválida. Faça login novamente. Código de suporte: req_auth_bad_token Detalhe: bearer_malformed',
    );
  });
  ...
  it('fails locally with a short diagnostic when the Pages runtime has no usable bearer token', async () => {
    ...
    await expect(request).rejects.toThrow(
      'Sua sessão expirou. Faça login novamente. Diagnóstico: auth_state_unusable',
    );
    expect(mockClearBrowserAuthState).toHaveBeenCalled();
    expect(fetchMock).not.toHaveBeenCalled();
  });

  it('allows unauthenticated admin requests during local development bypass', async () => {
    vi.resetModules();
    vi.stubEnv('VITE_LOCAL_DEV_BYPASS_AUTH', 'true');

    const bypassFetchMock = vi.fn().mockResolvedValue({
      ok: true,
      json: vi.fn().mockResolvedValue({ status: 'ok', transactions: [] }),
    });
    const bypassGetAccessToken = vi.fn().mockResolvedValue(null);
    const bypassClearBrowserAuthState = vi.fn();

    vi.stubGlobal('fetch', bypassFetchMock);

    vi.doMock('@/features/auth/lib/supabaseBrowserSession', () => ({
      clearBrowserAuthState: (...args: unknown[]) => bypassClearBrowserAuthState(...args),
      getAccessToken: (...args: unknown[]) => bypassGetAccessToken(...args),
    }));

    vi.doMock('@/features/observability/clientTelemetry', () => ({
    ...
    const { getTransactions: getTransactionsWithBypass } = await import('@/features/admin/api');

    await getTransactionsWithBypass();

    const [url, init] = bypassFetchMock.mock.calls[0] as [string, RequestInit];
    const headers = new Headers(init.headers);
    expect(url).toBe('/api/admin/gastos');
  ...
  it('emits client telemetry and surfaces a client support code when the browser cannot reach the backend', async () => {
    ...
    await expect(getTransactions()).rejects.toThrow(
      /Não foi possível conectar ao servidor agora\. Verifique sua conexão e tente novamente\. Código de suporte: cli_transport_1 Correlação: reqc_[a-f0-9]{1,24} Diagnóstico: frontend_transport_failed/i,
    );
  ...
  it('classifies cross-origin transport failures without treating them as confirmed CORS by default', async () => {
    ...
    const crossOriginFetchMock = vi.fn().mockRejectedValue(new TypeError('Failed to fetch'));
    const crossOriginGetAccessToken = vi.fn().mockResolvedValue('token-cross-origin');
    const crossOriginEmitClientTelemetry = vi.fn().mockReturnValue('cli_cross_origin_1');

    vi.stubGlobal('fetch', crossOriginFetchMock);

    vi.doMock('@/features/auth/lib/supabaseBrowserSession', () => ({
      clearBrowserAuthState: vi.fn(),
      getAccessToken: (...args: unknown[]) => crossOriginGetAccessToken(...args),
    }));

    vi.doMock('@/features/observability/clientTelemetry', () => ({
      emitClientTelemetry: (...args: unknown[]) => crossOriginEmitClientTelemetry(...args),
      ensureSupportCodeInMessage: (message: string, clientEventId?: string) =>
        clientEventId && !/c[oó]digo de suporte:/i.test(message)
          ? `${message} Código de suporte: ${clientEventId}`
          : message,
    ...
    expect(crossOriginEmitClientTelemetry).toHaveBeenCalledWith(expect.objectContaining({
      event: 'admin_api_transport_failed',
      phase: 'api_request',
      errorCode: 'NETWORK_ERROR',
      diagnostic: 'frontend_cross_origin_transport_failed',
