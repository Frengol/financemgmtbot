...
describe('clientTelemetry', () => {
  ...
  it('adds the client support code only once when formatting user-facing messages', async () => {
    ...
    expect(
      ensureSupportCodeInMessage('Falha temporária.', 'cli_support_1'),
    ).toBe('Falha temporária. Código de suporte: cli_support_1');
    expect(
      ensureSupportCodeInMessage('Falha temporária. Código de suporte: cli_support_1', 'cli_support_1'),
    ).toBe('Falha temporária. Código de suporte: cli_support_1');
  });
});
