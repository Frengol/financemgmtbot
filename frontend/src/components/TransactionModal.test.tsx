...
describe('TransactionModal', () => {
  ...
  it('creates a transaction with parsed currency input and emits refresh', async () => {
    ...
    await userEvent.type(valueInput, '4590');
    expect(valueInput.value).toBe('45,90');
    await userEvent.click(screen.getAllByRole('button', { name: 'Criar transação' })[0]);
  ...
  it('shows a session validation error before saving without access token', async () => {
    ...
    expect(await screen.findByText('Não foi possível validar sua sessão. Entre novamente.')).toBeInTheDocument();
  ...
  it('shows auth loading error before submitting', async () => {
    ...
    await userEvent.click(screen.getByRole('button', { name: 'Criar transação' }));
    expect(await screen.findByText('Aguarde a autenticação concluir antes de salvar.')).toBeInTheDocument();
  });
  ...
  it('surfaces backend save errors, supports local bypass and allows closing the modal', async () => {
    ...
    mockCreateTransaction.mockRejectedValueOnce(new Error('Não foi possível salvar agora.'));
  ...
  it('falls back to the generic save error when the backend rejects with a non-error value', async () => {
    ...
    expect(await screen.findByText('Não foi possível salvar a transação.')).toBeInTheDocument();
  });
});
