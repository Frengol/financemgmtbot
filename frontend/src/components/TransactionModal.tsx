...
export default function TransactionModal() {
  ...
  const title = editingId ? 'Editar transação' : 'Nova transação';

  const handleSubmit = async (event: React.FormEvent) => {
    event.preventDefault();

    if (authLoading) {
      setError('Aguarde a autenticação concluir antes de salvar.');
      return;
    }
    ...
    try {
      ...
      setError(submitError instanceof Error ? submitError.message : 'Não foi possível salvar a transação.');
    } finally {
      setSaving(false);
    }
  };
      ...
      <div className="w-full max-w-2xl rounded-2xl border border-slate-200 bg-white shadow-2xl">
        <div className="flex items-center justify-between border-b border-slate-200 px-6 py-4">
          <div>
            ...
            <p className="text-sm text-slate-500">Crie ou ajuste um lançamento manualmente no painel.</p>
          </div>
          <button
            type="button"
            onClick={close}
        ...
        <form onSubmit={handleSubmit} className="space-y-5 px-6 py-5">
          ...
          <div className="grid gap-4 md:grid-cols-2">
            ...
            <label className="space-y-1 text-sm">
              <span className="font-medium text-slate-700">Método de pagamento</span>
              <select
              ...
              >
                {paymentMethodOptions.map((option) => (
                  <option key={option} value={option}>{formatPaymentMethodLabel(option)}</option>
                ))}
              </select>
            </label>
            ...
            <label className="space-y-1 text-sm">
              ...
              >
                {accountOptions.map((option) => (
                  <option key={option} value={option}>{formatAccountLabel(option)}</option>
                ))}
              </select>
            </label>
          </div>
          ...
          <label className="block space-y-1 text-sm">
            ...
            <textarea
              ...
              maxLength={250}
              className="w-full rounded-xl border border-slate-200 bg-slate-50 px-3 py-2.5 text-slate-900 outline-none transition focus:border-blue-500 focus:bg-white"
              placeholder="Ex.: Almoço com cliente, farmácia, compras do mercado..."
              required
            />
          </label>
