  ...
  const authorizeAccessToken = useCallback(async (
    ...
    if (!isJwtShapeValid(accessToken)) {
      await persistAuthFailure(
        'Sua sessão de acesso é inválida. Faça login novamente. Diagnóstico: auth_state_unusable',
      );
      return;
    }
    ...
    try {
      ...
      if (!payload.authenticated || !payload.authorized) {
        throw new ApiError(
          'Seu usuário não está autorizado a acessar o painel.',
          {
      ...
      if (!authorizedUser) {
        throw new ApiError(
          'Não foi possível validar sua sessão agora. Faça login novamente.',
          {
      ...
      await persistAuthFailure(
        'Não foi possível validar sua sessão agora. Faça login novamente. Diagnóstico: auth_validation_failed',
      );
    }
